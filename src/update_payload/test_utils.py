...
def SignSha256(data, privkey_file_name):
    ...
    try:
        sign_process = subprocess.Popen(
            sign_cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE
        )
...
class PayloadGenerator(object):
    ...
    def _WriteHeaderToFile(self, file_obj, manifest_len):
        ...
        _WriteInt(
            file_obj,
            payload.Payload._PayloadHeader._MANIFEST_LEN_SIZE,
            True,
            manifest_len,
        )
...
class EnhancedPayloadGenerator(PayloadGenerator):
    ...
    def WriteToFileWithData(
        self, file_obj, sigs_data=None, privkey_file_name=None, padding=None
    ):
