...
def extract_img(zip_archive: zipfile.ZipFile, img_name, output_path, is_source):
    ...
    if file_size % 4096 != 0:
        if is_source:
            ...
            print("Rounding UP partition {} to a multiple of 4 KiB.".format(output_path))
...
def run_ota(source, target, payload_path, tempdir, output_dir):
    ...
    source_exist = source and (
        isinstance(source, zipfile.ZipFile) or os.path.exists(source)
    )
    target_exist = target and (
        isinstance(target, zipfile.ZipFile) or os.path.exists(target)
    )
    ...
    for part in payload.manifest.partitions:
        ...
        if part.HasField("old_partition_info"):
            assert source_exist, (
                "source target file must point to a valid zipfile or directory " + source
            )
    ...
    if not target_exist:
        ...
        return
    for expected_part, actual_part, part_name in zip(
        expected_new_partitions, new_partitions, partition_names
    ):
        if filecmp.cmp(expected_part, actual_part):
            ...
            valid = False
            print(
                "Partition `{}` is INVALID expected image: {} actual image: {}".format(
                    part_name, expected_part, actual_part
                )
            )
...
def main():
    ...
    parser.add_argument(
        "--source", help="Target file zip for the source build", required=False
    )
    parser.add_argument(
        "--target", help="Target file zip for the target build", required=False
    )
    parser.add_argument(
        "-o",
        dest="output_dir",
